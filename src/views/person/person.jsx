...
const Person = () => {
  ...
  $(document).off("click", "#editPerson").on("click", "#editPerson", function (e) {
    e.preventDefault(); // evitar que se produzca el evento por defecto
    var id = $(this).data("id");
    editarPersona(id);
    console.log(id);
  });
  ...
  const filtrar = async () => {
    const response = await getPersonasByFiltros(fields.tipO_DOC, fields.nuM_DOC);
    if (response.data) {
